...
function isXOutsideBorder(x: number, worldWidth: number): boolean {
  return x < -1 || x > worldWidth;
}

function isYOutsideBorder(y: number, worldHeight: number): boolean {
 ...
 */
export function isPointOutsideBorder(
  { x, y }: Point,
  { w, h }: GridSize,
): boolean {
...
export function isXOnLeftBorder(x: number): boolean {
  return x === -1;
}

/** Whether an x coordinate sits on the border column right of the grid (`w`). */
export function isXOnRightBorder(x: number, worldWidth: number): boolean {
  return x === worldWidth;
}

function isXOnBorder(x: number, worldWidth: number): boolean {
  return isXOnLeftBorder(x) || isXOnRightBorder(x, worldWidth);
}

/** Whether a y coordinate sits on the border row above the grid (`-1`). */
export function isYOnTopBorder(y: number): boolean {
  return y === -1;
}

/** Whether a y coordinate sits on the border row below the grid (`h`). */
export function isYOnBottomBorder(y: number, worldHeight: number): boolean {
  return y === worldHeight;
}

function isYOnBorder(y: number, worldHeight: number): boolean {
  return isYOnTopBorder(y) || isYOnBottomBorder(y, worldHeight);
}
...
export function isXInsideBorder(x: number, worldWidth: number): boolean {
  return x > -1 && x < worldWidth;
}

/** Whether a y coordinate lies strictly inside the grid (`0` to `h - 1`). */
export function isYInsideBorder(y: number, worldHeight: number): boolean {
  return y > -1 && y < worldHeight;
}

/** Whether a point lies strictly inside the grid — the playable area. */
export function isPointInsideBorder(
  { x, y }: Point,
  { w, h }: GridSize,
): boolean {
 ...
 */
export function gridContainsGrid(
